...
- FAQ publique inexacte (trouve le 2026-09-17) : `components/FaqSection.vue` (page d'accueil) affirme dans sa reponse "Comment fonctionne l'analyse par IA" que Stellara utilise GPT-4o. Faux d'apres le code actuel : `server/api/generate-report.post.ts` utilise exclusivement `claude-3-5-sonnet-latest` (Anthropic) pour generer le rapport natal ; GPT-4o-mini n'existe que comme fallback de `server/api/horoscope-today.get.ts` (localisation de l'horoscope du jour, feature differente). A corriger ou a confirmer aupres de l'utilisateur (peut-etre un texte laisse volontairement flou/ancien) avant toute autre page qui s'appuierait dessus.
