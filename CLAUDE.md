...
- Piege `public.siteUrl` (trouve le 2026-09-17, en prod) : le fallback par defaut de `public.siteUrl` dans `nuxt.config.ts` est deja `'http://localhost:3000'` (pas vide) quand `NUXT_PUBLIC_SITE_URL` n'est pas defini. Un simple `configuredUrl || fallback` dans un composable ne se declenche donc JAMAIS, meme en prod, car la valeur par defaut de Nuxt est deja une chaine non-vide. `composables/useSiteUrl.ts` reproduit maintenant la meme chaine de secours que `server/routes/sitemap.xml.ts` : valeur configuree (rejetee si elle ressemble a localhost) -> origine reelle de la requete via `useRequestURL()` (rejetee pareil) -> domaine prod en dur. Ca corrige le canonical (et le JSON-LD des articles) sans toucher au VPS, a condition que le reverse-proxy devant PM2 transmette bien `X-Forwarded-Proto`/`Host` -- verifie et confirme en local en simulant la requete (`curl -H "Host: stellara.sassify.fr" -H "X-Forwarded-Proto: https" ...`), pas teste en conditions reelles sur le VPS (pas d'acces SSH depuis cette session). Definir `NUXT_PUBLIC_SITE_URL` correctement dans l'env charge par PM2 reste une bonne pratique en plus, mais n'est plus une dependance stricte pour un canonical juste.
